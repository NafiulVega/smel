  ...
  for (JsonObject ch : doc["channels"].as<JsonArray>()) {
    ...
    if (relayState[idx] != newState) {
      Serial.printf("[RELAY] Ch%d (pin%d): %s -> %s\n", idx + 1, relayPins[idx],
                    relayState[idx] ? "ON" : "OFF", newState ? "ON" : "OFF");
...
// ============================================================
SensorReading readChannel(PZEM004Tv30 &pzem, HardwareSerial &serial, int rxPin,
                          int txPin, bool relayOn, bool inWarmup) {
  SensorReading r = {}; // zero-init semua field
  r.relay_on = relayOn;

  if (!relayOn)
    return r; // Relay OFF: tidak baca sensor
  ...
  if (isnan(v)) {
    ...
    // --- AUTO-RECOVERY UART ---
    // Jika terjadi error akibat kabel dicabut-pasang (hot-plug), driver UART
    // ESP32 bisa mengalami Framing Error dan buffer menjadi macet atau penuh
    // sampah. Kita bersihkan sisa data di buffer lalu restart port Serial untuk
    // memulihkan koneksi:
...
bool sendSensorData(SensorReading readings[]) {
  ...
  for (int i = 0; i < NUM_CHANNELS; i++) {
    ...
    if (readings[i].sensor_ok) {
      ...
      obj["voltage"] = obj["current"] = obj["power"] = obj["energy"] =
          obj["frequency"] = obj["pf"] = nullptr;
    }
  }

  // Tambahkan delay terakhir ke payload JSON agar bisa dibaca di Django (jika
  // diperlukan)
  ...
  HTTPClient http;
  String url =
      "http://" + String(SERVER_IP) + ":" + SERVER_PORT + "/api/sensor-data";
  http.setReuse(false); // Cegah koneksi TCP menggantung
...
void loop() {
  ...
  if (anyRelayOn) {
    ...
    readings[0] = readChannel(pzem1, SerialPZEM1, 16, 17, relayState[0],
                              relayWarmingUp[0]);
    readings[1] = readChannel(pzem2, SerialPZEM2, 18, 19, relayState[1],
                              relayWarmingUp[1]);
